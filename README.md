...
* [`src/cljs/re_frame_test/`](src/cljs/re_frame_test/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/re_frame_test/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `re-frame-test` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/re_frame_test/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns re-frame-test.example
  (:require [re-frame-test.config :as config])
